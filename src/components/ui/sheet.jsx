...
const sheetVariants = cva(
   "fixed z-50 gap-4 bg-background p-6 shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500 dark:bg-gray-950",
   {
      variants: {
         side: {
            left: "inset-y-0 left-0 h-full w-64 data-[state=closed]:slide-out-to-left data-[state=open]:slide-in-from-left",
...
const SheetContent = React.forwardRef(
   ({ side = "right", className, children, ...props }, ref) => (
      <SheetPortal>
         ...
         >
            {children}
            <SheetPrimitive.Close className="absolute right-4 top-4 rounded-sm text-white opacity-70 ring-2 ring-gray-300 ring-offset-1 ring-offset-gray-950 transition-opacity hover:opacity-100 focus:outline-none disabled:pointer-events-none data-[state=open]:bg-gray-800">
               <X className="h-5 w-5" />
